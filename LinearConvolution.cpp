...
#define rev_for(i,n) for (int i = (int)n; i>=0; i--)
...
int * linearConvolution(int x[],int h[],int y[],int n)
{
    repp(i,1,n)
    {
        int tmp = i;
...
int main()
{
    ...
    int x[] = {0,1,2,3,4,0,0,0};
    int h[] = {0,1,1,1,1,0,0,0};
...
*/
   /* while(cin.peek() != '\n'){
    ...
    int n = sizeof(x)/sizeof(x[0]) + sizeof(h)/sizeof(h[0]);
    int x_size  = sizeof(x)/sizeof(x[0]) ;
    int h_size = sizeof(h)/sizeof(h[0])
    //now need to resize x[] and h[]
    ...
    result = linearConvolution(x,h,y,n);
