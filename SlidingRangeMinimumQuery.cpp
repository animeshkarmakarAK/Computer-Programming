...
void RMQ(int m)
{
    ...
    {
        int leftMostElement = q.front().first;
        while(!q.empty() && leftMostElement > a[i]){
          q.pop_front();
          leftMostElement = q.front().first;
        }

        q.push_front(make_pair(a[i],i));
        int rightMostElementIndex = q.back().second;
        while(!q.empty() && rightMostElementIndex <= i-m)
        {
            //cout<<"Index::"<<rightMostElementIndex<<endl;
            q.pop_back();
            rightMostElementIndex = q.back().second;
        }
         if(i >= m -1)
