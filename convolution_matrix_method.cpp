...
void append(int x[],int x_size,int tmp[],int cnt)
{
...
void shift(int x[],int x_size,int tmp[],int cnt)
{
...
int main()
{
    ...
    int y[N];
    mem(y,0);
    mem(tmp,0);//tmp[0,0,0,0] now append and RC khao

    rep(i,N)
    {
        if(cnt < x_size)
           append(x,x_size,tmp,cnt);

       else {
        shift(x,x_size,tmp,cnt);
       }
